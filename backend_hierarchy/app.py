...
from model import label_encoder_hierarchial_rf_function
...
model_top3_1 = pickle.load(open("heirarchial_model_1.pkl", "rb"))
...
# Function to calculate cryptographic features
    ...
    if len(ciphertext_hex)==2:
        features['mean_acf'] = 0
        features['variance_acf'] = 0
        features['max_acf'] = 0
        features['lag_of_max_acf'] = 0
    ...
    byte_values = [int(c, 16) for c in ciphertext_hex]
...
# print(X)
import traceback
from model import byte_value_transition_matrix_entropy_imputer
@flask_app.route("/")
def Home():
...
def predict():
    """
    Handles prediction requests from the form or API.
    """
    ...
    try:
        ...
        X = features_df
        X['byte_value_transition_matrix_entropy'] = byte_value_transition_matrix_entropy_imputer().transform(X[['byte_value_transition_matrix_entropy']])
        ...
        from model import predict_with_top3_model_1
        prediction = predict_with_top3_model_1(X, model_top3_1, combination_models_1, all_classes)
        label_encoder_hierarchial_rf = label_encoder_hierarchial_rf_function()
        prediction = label_encoder_hierarchial_rf.inverse_transform(prediction)
        print(prediction)

        # Check if the request is from an API (Next.js frontend)
